...
float LQR_u = 0;
float angle_zeropoint = 5;  // 3.4 在理想平衡状态下俯仰角度，向前到正，向后倒负
...
FallDirection fall_direction = NOT_FALLEN;
...
void loop()
{
  ...
  // 倒地，但是倒地失控后被“扶起”，角度<25度，则不会进入条件
  if (abs(LQR_angle) > 25.0f)
  {
    uncontrolable = 1;
    if (LQR_angle > 25.0f) {
        fall_direction = FORWARD_FALL;
    } else if (LQR_angle < -25.0f) {
        fall_direction = BACKWARD_FALL;
    }
  }
  ...
  if (uncontrolable != 0) 
  {
      ...
      {
          uncontrolable = 0; // 可以输出
          fall_direction = NOT_FALLEN;
      }
      // ---------  被扶起, 角度小于10度

      // --------未被扶起，向后倒地时的自动爬起逻辑
      if (fall_direction == BACKWARD_FALL) {
          // 轮子突然加速向后
          motor1.target = 10.0; // 可根据实际情况调整
          motor2.target = 10.0; // 可根据实际情况调整
      } else {
          // 非向后倒地恢复状态，重置电机目标值和腿部控制量
          motor1.target = 0;
          motor2.target = 0;
          leg_position_add = 0;
      }
...
void web_loop()
{
  ...
  websocket.loop();
  if(bat_check_num > 200) // 200 数值更新的频率，越小越快
  {
    String dataA = String(LQR_angle, 2);
    String dataC = String(angle_control, 2);
    dataA += "---->";
    dataA += dataC;
    websocket.broadcastTXT(dataA);
  }
